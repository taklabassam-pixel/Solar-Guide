...
export function SolarPanelsCalculator({ 
  darkMode = true, 
  totalDailyWh = 0, 
  panelPower = 550, // 🟢 الحفاظ على قيمة اللوح الحالية القادمة من الأب
  onPanelPowerChange = () => {}, // 🟢 دالة التحديث
  onPanelConfigChange = () => {} 
}) {
  const { t, i18n } = useTranslation();
  const COMMON_PANEL_WATTAGES = [50, 100, 150, 200, 300, 350, 400, 450, 540, 550, 580, 650];

  const [dailyConsumption, setDailyConsumption] = React.useState(totalDailyWh || '');
  ...
  const panelResults = useMemo(() => {
    ...
    const wattUnit = t('calculators_hub.units.watt', t('units.watt', 'Watts'));
    const wattShortUnit = t('calculators_hub.units.watt_short', t('units.watt_short', 'W'));

    if (consumption <= 0) {
      const emptyConfig = { 
        totalPowerNeeded: `0 ${wattShortUnit}`, 
        panelsCount: t('solar_panels_calc.panels_unit', { count: 0, defaultValue: '0 panels' }), 
        singlePanelWatt, 
        note: null 
      };
    ...
    const config = {
      totalPowerNeeded: `${totalWattsNeeded} ${wattShortUnit}`,
      panelsCount: t('solar_panels_calc.panels_unit', { count, defaultValue: `${count} panels` }),
      singlePanelWatt: `${singlePanelWatt} ${wattUnit}`,
    ...
    <div className={`p-5 sm:p-6 rounded-2xl border shadow-sm space-y-5 transition-colors ${
      ...
      <div className={`flex items-center justify-between pb-4 border-b ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-bold text-base">{t('solar_panels_calc.header_title', 'حاسبة الألواح الشمسية')}</h3>
            <p className="text-[11px] opacity-60">{t('solar_panels_calc.header_subtitle', 'حساب قدرة المصفوفة والعدد المطلوب لملاحظة الأحمال')}</p>
          </div>
        </div>
        <span className={`text-[12px] font-bold px-2.5 py-1 rounded-md ${darkMode ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-600'}`}>
          {t('solar_panels_calc.badge_tag', 'الخطوة 2')}
        </span>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex justify-between items-center mb-1.5 flex-wrap gap-1">
            <label className="block text-xs font-bold opacity-80">
              {t('solar_panels_calc.daily_consumption_label', 'إجمالي الاستهلاك اليومي (واط.ساعة - Wh):')}
            </label>
            {totalDailyWh > 0 && (
              <button 
                type="button"
              ...
              >
                ⚡ {t('solar_panels_calc.fetch_calculated', { val: totalDailyWh.toLocaleString(), defaultValue: `جلب الحمل المحسوب (${totalDailyWh.toLocaleString()} Wh)` })}
              </button>
            )}
          </div>
          <input 
            ...
            placeholder={t('solar_panels_calc.placeholder_consumption', 'أدخل الاستهلاك')} 
        ...
        <div className="space-y-1">
          <label htmlFor="panelPower" className="block text-xs font-bold opacity-80">
            {t('solar_panels_calc.single_panel_watt_label', 'قدرة اللوح الوحيد المتاح (واط):')}
          </label>
          <select 
            id="panelPower"
            value={panelPower} 
            onChange={(e) => onPanelPowerChange(Number(e.target.value))} // 🟢 تحديث الـ State في الصفحة الرئيسية عند التغيير
          ...
          >
            {COMMON_PANEL_WATTAGES.map((watt) => (
              <option key={watt} value={watt}>
                {watt} {t('calculators_hub.units.watt', t('units.watt', 'Watts'))}
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="space-y-2 pt-2">
          <div className="grid grid-cols-2 gap-3">
            <div className={`p-3 rounded-xl border ${darkMode ? 'bg-amber-500/10 border-amber-500/20 text-amber-300' : 'bg-amber-50 border-amber-200 text-amber-950'}`}>
              <span className="block text-[11px] font-bold opacity-80">{t('solar_panels_calc.total_required_power', 'إجمالي القدرة المطلوبة:')}</span>
            ...
            <div className={`p-3 rounded-xl border ${darkMode ? 'bg-sky-500/10 border-sky-500/20 text-sky-300' : 'bg-sky-50 border-sky-200 text-sky-950'}`}>
              <span className="block text-[11px] font-bold opacity-80">{t('solar_panels_calc.suggested_panels_count', 'الألواح الموصى بها:')}</span>
